...
void generarXML(char* nombreRed, char* ip, char* mascaraRed, char* puertaEnlace, char* dns1, char* mediaMs, char* saltos) {
	...
	fprintf(fp, "<?xml-stylesheet type = \"text/xsl\" href = \"style.xslt\"?>\n"); //indicamos el fichero de estilos XSLT
	...
	printf("\n\n");
	printf("Fichero XML generado correctamente (enlazado con XSLT).\n\n");
	system("pause");
	system("cls");
}
...
void generarXSLT() {
	...
	printf("\n\n");
	//printamos la información que se va a exportar al fichero XML
	printf("Se va a generar una hoja de estilo XSLT\n\n");
	system("pause");
	fp = fopen("style.xslt", "w");
	if (fp == NULL) {
		perror("Error al abrir el fichero");
		return;
	}
	fprintf(fp, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"); //línea para indicar el tipo de "encoding", debe ser UTF-8 para un correcto visionado en el navegador
	fprintf(fp, "<xsl:stylesheet version=\"1.0\" xmlns:xsl = \"http://www.w3.org/1999/XSL/Transform\">\n"); 
	fprintf(fp, "<xsl:template match =\"/\">\n");
	fprintf(fp, "<html>\n");
	fprintf(fp, "<head>\n");
	fprintf(fp, "<title>Información del Adaptador</title>\n");
	fprintf(fp, "</head>\n");
	fprintf(fp, "<body>\n");
	fprintf(fp, "<h1 style=\"font-family: Arial; color: #1a5276; text-align: center; font-style: italic;\">Producto 5 - Transformación de un archivo XML mediante una hoja de estilo XSLT.</h1>\n");
	fprintf(fp, "<h2 style=\"font-family: Arial; color: #2e86c1; text-align: center;\">Información del adaptador</h2>\n");
	fprintf(fp, "<table border=\"1\" style=\"width: 80%%; margin: 0 auto; border-collapse: collapse; font-family: Arial;\">\n");
	fprintf(fp, "<tr style=\"background-color: #f8c471; text-align: left;\">\n");
	fprintf(fp, "<th>Elemento</th>\n");
	fprintf(fp, "<th>Parámetro</th>\n");
	fprintf(fp, "</tr>\n");
	fprintf(fp, "<tr>\n");
	fprintf(fp, "<td>Nombre</td>\n<td><xsl:value-of select=\"adaptador/nombre\" /></td>\n");
	fprintf(fp, "</tr>\n");
	fprintf(fp, "<tr>\n");
	fprintf(fp, "<td>IP</td>\n<td><xsl:value-of select=\"adaptador/ip\" /></td>\n");
	fprintf(fp, "</tr>\n");
	fprintf(fp, "<tr>\n");
	fprintf(fp, "<td>Máscara de red</td>\n<td><xsl:value-of select=\"adaptador/mascara\" /></td>\n");
	fprintf(fp, "</tr>\n");
	fprintf(fp, "<tr>\n");
	fprintf(fp, "<td>Puerta de enlace</td>\n<td><xsl:value-of select=\"adaptador/puerta\" /></td>\n");
	fprintf(fp, "</tr>\n");
	fprintf(fp, "<tr>\n");
	fprintf(fp, "<td>DNS primaria</td>\n<td><xsl:value-of select=\"adaptador/dns\" /></td>\n");
	fprintf(fp, "</tr>\n");
	fprintf(fp, "<tr>\n");
	fprintf(fp, "<td>Media de respuesta</td>\n<td><xsl:value-of select=\"adaptador/media\" /></td>\n");
	fprintf(fp, "</tr>\n");
	fprintf(fp, "<tr>\n");
	fprintf(fp, "<td>Cantidad de saltos</td>\n<td><xsl:value-of select=\"adaptador/saltos\" /></td>\n");
	fprintf(fp, "</tr>\n");
	fprintf(fp, "</table>\n");
	fprintf(fp, "</body>\n");
	fprintf(fp, "</html>\n");
	...
	printf("\n\n");
	printf("Fichero XSLT generado correctamente.\n\n");
	system("pause");
	system("cls");
}